...
public class ServerHandler implements ServerHandlerInterface{
	...
	public boolean createForum(String forumName, String userName, String hasEmailPolicy, String extendedDeletionPolicy,String minPostForModerator, String minSeniorityMonths, String onlyApointAdministratorCanRemoveModerators, String canRemoveSingleModerators, String expirationDateForPassword, String interactiveNotifyingPolicys){
		if (userName.compareTo(Configuration.superAdminName)==1){
			System.out.println("--------------"+userName.compareTo(Configuration.superAdminName));
			return false;
		}	
	...
	public boolean publishNewThread(String forumName, String subForumName,
		...
		if (ans){
			for(String m:mem.getFriends()){
				this.forumSystem.getForum(forumName).findUser(m).notifyMember("your friend: "+userName
						+" in forum: "+forumName+"has published a new thread in subforum: "+subForumName);
			}
			return true;
		}
			return false;
	...
	public boolean editTread(String userName, String forum, String subForum, String treadID, String newText) {
		...
		if (userName.compareTo(thread.getMember())==0){
			...
			for(AbstractPost post:thread.getFollowPosts()){
				this.forumSystem.getForum(forum).findUser(post.getMember()).notifyMember(userName+" changed post titled "+thread.getTitle()+
						"in forum: "+forum+", subforum: "+subForum);
			}
			return true;
		}
		return false;
	...
	// 43
	public boolean responseToFreindRequest(String forumName, String userName, String friendUserName, String answer) {
		Forum forum = this.forumSystem.getForum(forumName);
		Member member = forum.getMember(userName);
		if (answer.contains(Configuration.SUCCESS))
			return member.addFriend(friendUserName);
		return true;
	}
	...
	public boolean sendFriendRequest(String requesterName,String sendTo,String forumName){
		if(!isMember(requesterName, forumName) || !isMember(sendTo, forumName) || requesterName.contentEquals(sendTo))
			return false;
		boolean ans=this.forumSystem.getForum(forumName).findUser(sendTo).addFriendrequest(requesterName);
		if(ans)
			this.forumSystem.getForum(forumName).findUser(sendTo).notifyMember("The Member: " + requesterName +" sent a friend request in forum: "+forumName);
		return ans;
	...
	public Vector<String> getNotifications(String userName,String forumName) {
		return this.forumSystem.getForum(forumName).findUser(userName).clearNotification();
	}
