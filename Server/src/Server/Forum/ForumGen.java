...
public class ForumGen {
	...
	public ForumGen() {
		...
		this.forums = new Vector<Forum>();//fh.getForums();//
	...
	public Forum createForum(String forumName, Policy policy){
		...
		this.forumsHandler.addForum(forumName, policy.hasEmailConfirmation(), policy.getExtendedDeletionPolicy(),
									policy.getMinSeniorityMonths(),policy.getMinPostForModerator(),
									policy.getOnlyApointAdministratorCanRemoveModerators(),
									policy.getCanRemoveSingleModerators(), policy.getMonthsPasswordValidFor(),
									policy.getInteractiveNotifyingPolicys());
