package ListenerServer.reactor;
...
import ListenerServer.protocol.*;
...
public class ConnectionHandler<T> {
	 ...
	 */
	private ConnectionHandler(SocketChannel sChannel, ReactorData<T> data, SelectionKey key, Client client) {
		_sChannel = sChannel;
		_data = data;
...
//		} catch (IOException e) {System.out.println("sendAll faild");}
	...
	private void initialize() {
		_skey.attach(this);
		_task = new ProtocolTask<T>(_protocol, _tokenizer, this, _client);
	}

	public static <T> ConnectionHandler<T> create(SocketChannel sChannel, ReactorData<T> data, SelectionKey key,Client client) {
		ConnectionHandler<T> h = new ConnectionHandler<T>(sChannel, data, key, client);
		h.initialize();
		return h;
	}
